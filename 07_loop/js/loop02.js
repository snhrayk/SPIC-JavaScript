let cities = [
  { name: "大阪市", population: 2756034 },
  { name: "堺市", population: 822671 },
  { name: "東大阪市", population: 489151 },
  { name: "豊中市", population: 401233 },
  { name: "枚方市", population: 398287 },
  { name: "吹田市", population: 378171 },
  { name: "高槻市", population: 350642 },
  { name: "茨木市", population: 283155 },
  { name: "八尾市", population: 264203 },
  { name: "寝屋川市", population: 229286 },
  { name: "岸和田市", population: 191439 },
  { name: "和泉市", population: 184659 },
  { name: "守口市", population: 143323 },
  { name: "箕面市", population: 136277 },
  { name: "門真市", population: 119795 },
];
const result = document.getElementById("result");

const forty = cities.filter((c) => {
  // console.log(c.name, c.population);
  return c.population >= 400000;
});
// cの中にはcitiesのnameとpopulationが入っている

for (let i = 0; i < forty.length; i++) {
  // console.log(forty[i].name);
  result.innerText += forty[i].name;
}
